...
(define (make-test-suite name run-proc)
  (test-suite
   ...
   (test-suite
    "Midterm"
    (check-equal? (run-proc '(string->list "")) '())
    (check-equal? (run-proc '(string->list "wilma")) '(#\w #\i #\l #\m #\a))
    (check-equal? (run-proc '(string->list 42)) 'err)
    (check-equal? (run-proc '(string-copy "")) "")
    (check-equal? (run-proc '(string-copy #\a)) 'err)
    (check-equal? (run-proc '(let ((x "wilma")) (eq? (string-copy x) x))) #f)
    (check-equal? (run-proc '(let ((x (string-copy "wilma"))) (let ((y (string-copy "fred"))) x ))) "wilma")
    (check-equal? (run-proc '(let ((x (string-copy "fred"))) (begin (string-copy "wilma") x))) "fred")
    (check-equal? (run-proc '(string-append)) "")
    (check-equal? (run-proc '(string-append "")) "")
    (check-equal? (run-proc '(string-append "wilma")) "wilma")
    (check-equal? (run-proc '(string-append "wilma" " and " "fred")) "wilma and fred")
    (check-equal? (run-proc '(string-append "hello" 13)) 'err)
    (check-equal? (run-proc '(let ((x "wilma")) (string-append x x))) "wilmawilma")
    (check-equal? (run-proc '(let ((x "wilma")) (string-append "fred" x))) "fredwilma")
    (check-equal? (run-proc '(let ((x "")) (string-append x x))) "")
    (check-equal? (run-proc '(string-append "12345" "abcd")) "12345abcd")
    (check-equal? (run-proc '(let ((x "wilma")) (string-append "" x))) "wilma")
    (check-equal? (run-proc '(let ((x "wilma")) (string-append x ""))) "wilma")
    (check-equal? (run-proc '(let ((x "wilma")) (eq? (string-append "" x) x))) #f)
    (check-equal? (run-proc '(let ((x "wilma")) (eq? (string-append x "") x))) #f)
    (check-equal? (run-proc '(let ((x (string-append "wilma"))) (begin "foo" x))) "wilma"))
