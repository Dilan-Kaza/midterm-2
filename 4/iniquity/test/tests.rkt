...
(define (make-test-suite name run-proc)
  (test-suite
   ...
   (test-suite
    "Midterm"
    (check-equal? (run-proc '(define (f) 42) '(f)) 42)
    (check-equal? (run-proc '(define (f x) x) '(f 42)) 42)
    (check-equal? (run-proc '(define (f [x 42]) x) '(f)) 42)
    (check-equal? (run-proc '(define (f [x 42]) x) '(f 17)) 17)
    (check-equal? (run-proc '(define (f x y [z 3]) (+ x (+ y z))) '(f)) 'err)
    (check-equal? (run-proc '(define (f x y [z 3]) (+ x (+ y z))) '(f 1)) 'err)
    (check-equal? (run-proc '(define (f x y [z 3]) (+ x (+ y z))) '(f 1 2)) 6)
    (check-equal? (run-proc '(define (f x y [z 3]) (+ x (+ y z))) '(f 1 2 4)) 7)
    (check-equal? (run-proc '(define (f x y [z 3]) (+ x (+ y z))) '(f 1 2 4 5)) 'err)
    (check-equal? (run-proc '(define (f x [y 2] [z 3]) (+ x (+ y z))) '(f)) 'err)
    (check-equal? (run-proc '(define (f x [y 2] [z 3]) (+ x (+ y z))) '(f 1)) 6)
    (check-equal? (run-proc '(define (f x [y 2] [z 3]) (+ x (+ y z))) '(f 1 8)) 12)
    (check-equal? (run-proc '(define (f x [y 2] [z 3]) (+ x (+ y z))) '(f 1 8 10)) 19)
    (check-equal? (run-proc '(define (f x [y 2] [z 3]) (+ x (+ y z))) '(f 1 8 10 37)) 'err))
