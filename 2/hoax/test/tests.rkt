...
(define (make-test-suite name run-proc)
  (test-suite
   ...
   (test-suite
    "Midterm"
    (check-equal? (run-proc '(vector-append (vector) (vector))) (vector))
    (check-equal? (run-proc '(vector-append (vector) (vector 1 2 3))) (vector 1 2 3))
    (check-equal? (run-proc '(vector-append (vector 1 2 3) (vector))) (vector 1 2 3))
    (check-equal? (run-proc '(vector-append (vector 1 2 3) (vector 4 5 6))) (vector 1 2 3 4 5 6))
    (check-equal? (run-proc '(vector-append (list) (vector))) 'err)
    (check-equal? (run-proc '(vector-append (vector) (list))) 'err)
    (check-equal? (run-proc '(vector-append 42 #\a)) 'err)
    (check-equal? (run-proc '(let ([v (vector 1 2 3)]) (vector-append v v))) (vector 1 2 3 1 2 3)))
